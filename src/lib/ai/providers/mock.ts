...
export const mockProvider: AIProvider = {
  ...
  async extractDocument(req: AIDocumentExtractionRequest): Promise<AIDocumentExtractionResponse> {
    const start = Date.now();
    // Sans clé IA réelle, on ne peut pas lire une image. Si du texte dicté
    // est fourni, on applique une extraction heuristique simple pour que le
    // flux reste testable de bout en bout en mode mock.
    const text = req.text ?? "";
