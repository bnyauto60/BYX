        ...
        if (auth?.user) {
                    ...
                    if (profile?.workshop_id) {
                                    const { data: workshop } = await supabase
                                        .from("workshops")
                                        .select("name, city, ai_provider")
                                        .eq("id", profile.workshop_id)
                                        .single();
                                    if (workshop?.ai_provider) currentAiProvider = workshop.ai_provider;
                                    if (workshop?.name) workshopName = workshop.name;
                                    if (workshop?.city) workshopCity = workshop.city;
                    }
        }

    const workshopLabel = workshopName || workshopCity
            ? `${workshopName}${workshopName && workshopCity ? " - " : ""}${workshopCity}`
                : "Atelier non renseigne.";
